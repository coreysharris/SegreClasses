...
projectiveDegrees(Ideal,Ideal) := opts -> (I,J) -> (
    ...
    IA := intersectionRing(ChowRingg R,n);
    Y := scheme(J,IA);
    X := scheme(I+J,IA);
    X.equations = I;
...
isMultiHomo=method(TypicalValue=>Boolean);
isMultiHomo Ideal:=I->(
...
isMultiHomo RingElement:=f->(
    return isMultiHomo(ideal(f));
);
...
MultiProjCoordRingg=method(TypicalValue=>Ring);
MultiProjCoordRingg (Symbol,List):=(x,l)->(
    kk:=ZZ/32749;
    return MultiProjCoordRingg(kk,x,l);
);
MultiProjCoordRingg (Ring,List):=(kk,l)->(
    x:=getSymbol "x";
    return MultiProjCoordRingg(kk,x,l);
);
MultiProjCoordRingg (List):=(l)->(
    x:=getSymbol "x";
    kk:=ZZ/32749;
    return MultiProjCoordRingg(kk,x,l);
);
MultiProjCoordRingg (Ring, Symbol,List):=(kk,x,l)->(
...
ChowRingg=method(TypicalValue=>QuotientRing);
ChowRingg (Ring):=(R)->(
    h := getSymbol "h";
    return ChowRingg(R,h);
);
ChowRingg (Ring,Symbol):=(R,h)->(
    Rdegs:=degrees R;
    ChDegs:=unique Rdegs;
    m:=length ChDegs;
...
segre (Ideal,Ideal,QuotientRing) :=opts->(X,Y,A) -> (
    if not isMultiHomo(X) then (print "the first ideal is not multi-homogenous, please correct this"; return 0;);
    if not isMultiHomo(Y) then (print "the second ideal is not multi-homogenous, please correct this"; return 0;);
...
InterProd (Ideal,Ideal,Ideal,QuotientRing) :=opts->(Ix,Iv,Iy,A) -> (
    ...
    S:= MultiProjCoordRingg(prodSpaceNs);
	...
	if temp!={} then(
	    pfSeg=pfSeg+lift((flatten entries last coefficients first temp)_0,ZZ)*A_0^(numgens(R)-i-1);
	    );
	);
...
A = ZZ[a,b,Degrees=>{{1,0},{0,1}}]/(a^4,b^4)
time s=segre(Cx,I,A,Verbose=>true)
...
kk=ZZ/32749
R = kk[x,y,z,t]
PP3xPP3 = MultiProjCoordRingg(kk,{3,3})
